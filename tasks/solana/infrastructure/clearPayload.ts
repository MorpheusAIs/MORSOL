...
task("morpheus:oft:solana:clear", "Clear a stored payload on Solana")
  ...
  .addParam(
    "receiver",
    "The receiver address on the destination Solana chain (bytes58)",
    undefined,
    types.string,
  )
  ...
  .addParam(
    "computeUnits",
    "The CU for the lzReceive instruction",
    undefined,
    types.int,
  )
  .addParam(
    "lamports",
    "The lamports for the lzReceive instruction",
    undefined,
    types.int,
  )
  .addParam(
  ...
  .setAction(
    async ({
      ...
      if (!process.env.SOLANA_PRIVATE_KEY) {
        throw new Error(
          "SOLANA_PRIVATE_KEY is not defined in the environment variables.",
        );
      }
