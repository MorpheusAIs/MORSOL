...
import {
  getSolanaReceiveConfig,
  getSolanaSendConfig,
} from "../../utils/solana/infrastructure/helpers";
import { createSolanaConnectionFactory } from "../../utils/solana/infrastructure/helpers";
 ...
 */
const isSolana = (point: OmniPoint) =>
  endpointIdToChainType(point.eid) === ChainType.SOLANA;
...
const action: ActionType<TaskArgs> = async (
  ...
  const tasks = graph.connections
    ...
    .map(({ vector: { from, to } }) => async () => {
      ...
      const sendCustomConfig = await getSendConfig(
        endpointV2Sdk,
        to.eid,
        from.address,
        true,
      );
      const [sendCustomLibrary, sendCustomUlnConfig, sendCustomExecutorConfig] =
        sendCustomConfig ?? [];
      const [receiveCustomLibrary, receiveCustomUlnConfig] =
        receiveCustomConfig ?? [];

      // Default Config
      const receiveDefaultConfig = await getReceiveConfig(
        endpointV2Sdk,
        to.eid,
      );
      ...
      const [
        ...
        sendDefaultExecutorConfig,
      ] = sendDefaultConfig ?? [];
      const [receiveDefaultLibrary, receiveDefaultUlnConfig] =
        receiveDefaultConfig ?? [];

      // OApp Config
      const receiveOAppConfig = await getReceiveConfig(
        endpointV2Sdk,
        to.eid,
        from.address,
      );
      const sendOAppConfig = await getSendConfig(
        endpointV2Sdk,
        to.eid,
        from.address,
      );
      const [sendOAppLibrary, sendOAppUlnConfig, sendOAppExecutorConfig] =
        sendOAppConfig ?? [];
      const [receiveOAppLibrary, receiveOAppUlnConfig] =
        receiveOAppConfig ?? [];
  ...
  const solTasks = graph.connections
    ...
    .map(({ vector: { from, to } }) => async () => {
      ...
      // OApp Config
      const receiveOAppConfig = await getSolanaReceiveConfig(
        endpointV2Sdk,
        to.eid,
        from.address,
      );
      const sendOAppConfig = await getSolanaSendConfig(
        endpointV2Sdk,
        to.eid,
        from.address,
      );
